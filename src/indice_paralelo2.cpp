...
int leer_variable_entorno(const char* nombre, int valor_defecto) {
    const char* valor_str = getenv(nombre);
    if (valor_str == nullptr) {
        cout << "[CONFIG] Variable de entorno " << nombre << " no encontrada. Usando valor por defecto: " << valor_defecto << endl;
        return valor_defecto;
    }
    try {
        int valor = std::stoi(valor_str);
        cout << "[CONFIG] Variable de entorno " << nombre << " encontrada. Usando valor: " << valor << endl;
        return valor;
    } catch (const std::exception& e) {
        cerr << "[ERROR] Error al parsear " << nombre << " (" << valor_str << "). Usando valor por defecto: " << valor_defecto << endl;
        return valor_defecto;
    }
}
...
AppConfig configurar_aplicacion(int argc, char* argv[]) {
    ...
    config.N_LOTE = leer_variable_entorno("N_LOTE", 10);
